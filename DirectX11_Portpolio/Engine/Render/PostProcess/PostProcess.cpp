                                            ...
                                            const vector<ComPtr<ID3D11RenderTargetView>>& targets) 
{
    ...
    for (int i = 0; i < BloomLevels - 1; i++) 
    {
        ...
        BloomDownFilters[i].Initialize(EPostProcessType::BloomDown, width / div, height / div);
        if (i == 0) 
        {
    ...
    for (int i = 0; i < BloomLevels - 1; i++) 
    {
        ...
        BloomUpFilters[i].Initialize(EPostProcessType::BloomUp, width / div, height / div);
        BloomUpFilters[i].SetShaderResources({ bloomSRVs[level + 1] });
        BloomUpFilters[i].SetRenderTargets({ bloomRTVs[level] });
    }

    // Combine + ToneMapping
    CombineFilter.Initialize(EPostProcessType::Combine, width, height);
