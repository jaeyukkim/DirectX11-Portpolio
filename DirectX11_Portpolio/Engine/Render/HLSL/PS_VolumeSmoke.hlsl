...
Texture3D<float> densityTex : register(t30); // t5 부터 시작
Texture3D<float> lightingTex : register(t31);
Texture3D<float> temperatureTex : register(t32);
...
float4 PS_Main(VertexOutput input) : SV_TARGET
{
    ...
    float3 eyeModel = mul(float4(EyePos, 1), WorldInv).xyz; // 월드->모델 역변환
    float3 dirModel = normalize(input.posModel - eyeModel);

    int numSteps = 128;
    float stepSize = BoxHalfWidth * 2.0 / float(numSteps);
    ...
    float3 posModel = input.posModel + dirModel; // 살짝 들어간 상태에서 시작
    ...
    {
        float3 uvw = GetBoxUVW(posModel, BoxHalfWidth); // +uvwOffset; 미사용 
        ...
        if (abs(posModel.x) > BoxHalfWidth || abs(posModel.y) > BoxHalfWidth || abs(posModel.z) > BoxHalfWidth)
            break;
