...
void AController::Possess(ACharacter* InCharacter)
{
...
void AController::AddRotationInput(Vector3 InVal)
{
...
void AController::AddMovementInput(const Vector3& moveVal)
{
    PendingMovementInput += moveVal;
}

void AController::Jump()
{
...
void AController::InitPhysController()
{
    CheckNull(ControlledCharacter);
    ...
    PhysController->getActor()->userData = static_cast<void*>(ControlledCharacter);
}


void AController::AddMovementToPhys(float deltaTime)
{
    CheckNull(PhysController.get());
